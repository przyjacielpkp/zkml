...
use luminal::op::{Add, LessThan, Mul, Recip};
...
pub fn snark_input_mapping(
  ...
  assignment.into_iter().for_each(|(k, v)| match v {
    Some(vv) => inputs_tracker.new_inputs[&k]
      ...
      .zip(vv)
      .for_each(|(x, a)| {
        result.insert(*x, SourceType::scaled_private(a, &scale));
      }),
    None => inputs_tracker.new_inputs[&k].iter().for_each(|x| {
      result.insert(*x, SourceType::Private(None));
    }),
  });
  result
}
...
impl MLSnark<CircuitField> {
  ...
  pub fn get_evaluation_result(&self) -> CircuitField {
    *self.recorded_public_inputs.last().unwrap()
  }
...
fn set_input(source_map: &mut SourceMap, tracker: &InputsTracker, id: NodeIndex, value: Vec<f32>) {
  ...
  for (little_id, v) in little_ids.iter().zip(value) {
  ...
  fn generate_constraints(
    ...
    for x in pi {
      ...
      let (v, ass) = {
        ...
        // UNOP
        else if let Some((((_, _, _), _),)) = incoming.iter().collect_tuple() {
        ...
        // BINOP
          ...
          let ll = vars.get(&l).unwrap();
          let rr = vars.get(&r).unwrap();
          ...
          if graph.check_node_type::<Add>(x) {
            ...
            let lt_ass = lt_ass_bool.map(|b| if b { BigInt::from(1) } else { BigInt::from(0) });

            let make_xy = |noneg| {
              let ass = lt_ass_bool.and_then(|b| {
                lr.clone()
            ...
            let xxx = FpVar::<Fr>::Var(AllocatedFp::new(
              x_val.and_then(|x| f_from_bigint(x.clone()).ok()),
              x,
              cs.clone(),
            ));
            let yyy = FpVar::<Fr>::Var(AllocatedFp::new(
              y_val.and_then(|x| f_from_bigint(x.clone()).ok()),
              y,
              cs.clone(),
            ));
        ...
        .is_none()
      {
        ...
        if let Some(n) = ass.clone() {
          public_record.push(f_from_bigint_unsafe(n))
        }
      }
    }
